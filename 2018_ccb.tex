...
\begin{itemize}
  ...
  \item While relevant estimation procedures are usually data-intensive,
    variance moderation stabilizes their small-sample properties, unlocking
    their use for differential expression analyses.
  \item Focusing on a targeted maximum likelihood estimator (TMLE) of the
    average treatment effect (ATE), we illustrate how the approach generalizes
    to asymptotically linear estimator.
  \item Supervised clustering is discussed in the context of influence
    functions, ...
  \item The \underline{\texttt{biotmle} R package} \cite{hejazi2017biotmle}
    implements the inference and clustering approaches while leveraging
...
\begin{itemize}
  ...
  \item A \textit{supervised distance matrix} \cite{pollard2008supervised}
    may be constructed by applying a distance metric of choice (e.g., Euclidean,
    correlation) to the transformed values $Z$.
  \item $\widetilde{T}(Z)$, the resultant $B \times B$ empirical distance
    ...
    profiles.
  \item When $\widetilde{T}(b,b') = g(Z_b, Z_{b'})$ is small, biomarkers $b$ and
    $b'$ have similar associations between expression and the the target
    parameter $\Psi$, across the $n$ subjects.
...
\begin{center}
\vspace*{-0.5cm}
\includegraphics[scale=0.30]{topGenesHeatmap}
...
\includegraphics[scale=0.2]{topGenesHeatmap}
